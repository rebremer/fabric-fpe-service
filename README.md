## Mask data using format preserved encryption (FPE)

This repo demonstrates a scalable PII masking capability based on [format preserving encryption](https://en.wikipedia.org/wiki/Format-preserving_encryption), used to mask data flowing from PRD to lower (TST/DEV) environments. FPE replaces sensitive values with cipher text that keeps the same shape (length, character classes, separators), so downstream systems and schemas keep working unchanged. For example:
...
Two deployment patterns are provided:

| Option | Folder | Where the FPE key/tweak live | When to pick it |
|---|---|---|---|
| **1. FPE-as-a-Service (Fabric)** | [service/](service/) | Live **only** inside an Azure Function App (Key Vault references). The Spark cluster holds nothing but an HTTP function key and calls a batched `pandas_udf`. | Key custodianship must be separated from the data platform team; multiple Spark workspaces / engines need to share the same FPE; rotate function key independently of the FPE key. |
| **2. Direct keys in cluster (Synapse / Fabric)** | [direct_keys/](direct_keys/) | Loaded into every Spark executor (from Key Vault) at session start. Includes Azure Synapse and Microsoft Fabric notebooks plus a Synapse orchestration pipeline. | Single team owns both the keys and the Spark workspace; lowest latency; no extra service to operate. |

The primary, recommended pattern is **option 1 (FPE-as-a-Service)** and is described next. The legacy direct-keys Synapse variant is kept as an [appendix](#appendix-direct-keys-in-cluster--synapse-variant) at the end.

---

## FPE-as-a-Service (Azure Function) – Fabric variant

The notebooks under `direct_keys/Synapse/` and `direct_keys/Fabric/` perform FPE *inside* the Spark cluster, which means the FF3 key/tweak material has to be loaded into every executor (typically pulled from Key Vault on session start). This variant moves the FPE computation into an **Azure Function App** so that **no key material ever lives on the Spark cluster** — the cluster only holds an HTTP function key.
...
| Path | Purpose |
|---|---|
| `service/azure_function/function_app.py`     | Python v2 Functions app. Single endpoint `POST /api/fpe`. Batched FF3 with deterministic alphabets per type. |
| `service/azure_function/requirements.txt`    | `azure-functions`, `ff3==1.0.3`, `pycryptodome==3.23.0`, `unidecode>=1.3` |
| `service/azure_function/host.json`           | Functions host config (extension bundle 4.x). |
| `service/deployment/deployment.ps1`          | End-to-end PowerShell script: RG, VNet + delegated subnet, storage account with shared keys disabled, Private Endpoints (blob/queue/table) + Private DNS, Flex Consumption Function App with system-assigned MI + identity-based `AzureWebJobsStorage`, app settings, publish, lock-down. |
| `service/notebooks/mask_data_fpe_ff3.ipynb`  | Fabric notebook that calls the function via a batched `pandas_udf` and scales a 1k-row seed dataset to 1M rows for a perf test. |
...
```powershell
# 1. Edit service/deployment/deployment.ps1 to set $rg/$loc/$stor/$func names, then run end-to-end.
.\service\deployment\deployment.ps1
```
...
## Appendix: Direct keys in cluster — Synapse variant

This is the original Azure Synapse implementation, kept for reference. Here the FF3 key/tweak are pulled from Key Vault into the Spark session and live inside every executor.
...
- Azure Key vault: This Keyvault will be used to store the secrets that are used by Synapse to create the HMACs in ```direct_keys/Synapse/mask_data_fpe_prefixcipher.ipynb``` and encryption in ```direct_keys/Synapse/mask_data_fpe_ff3.ipynb```
...
Pipeline can be found in  ```direct_keys/Synapse/synapse_pipeline.json```
